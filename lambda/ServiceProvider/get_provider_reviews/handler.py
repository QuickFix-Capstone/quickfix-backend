...
def handler(event, context):
    """
    Lambda entrypoint for getting all reviews for a provider.
    
    Path Parameters:
        provider_id: The provider's ID (from URL path)
    ...
    Example URLs:
        ...
        GET /reviews/provider/SP-001?sort=highest_rating&limit=20&offset=0
    ...
    """
    # 1) Extract provider_id from path parameters
    path_params = event.get("pathParameters") or {}
    provider_id = path_params.get("provider_id")
    
    # For local testing, allow provider_id in event root
    if not provider_id:
        provider_id = event.get("provider_id")
    
    if not provider_id:
        return _response(400, {"message": "Missing required path parameter: provider_id"})
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 6) Get provider's rating summary
            summary_sql = """
                SELECT 
                    average_rating,
                    total_review_count
                ...
                WHERE provider_id = %s
            """
            cur.execute(summary_sql, (provider_id,))
            summary_result = cur.fetchone()
            
            if not summary_result:
                return _response(404, {"message": "Provider not found"})
            ...
            cur.execute(reviews_sql, (provider_id, limit, offset))
...
if __name__ == "__main__":
    print("=" * 70)
    print("Testing GET /reviews/provider/{provider_id}")
    print("=" * 70)
    
    # Test 1: Get reviews for SP-001
    print("\n📋 Test 1: Get reviews for SP-001 (default params)")
    print("-" * 70)
    test_event_1 = {
        "provider_id": "SP-001",
        "sort": "newest",
    ...
    print("\n📋 Test 2: Get reviews for SP-001 (sorted by highest rating)")
    print("-" * 70)
    test_event_2 = {
        "provider_id": "SP-001",
        "sort": "highest_rating",
