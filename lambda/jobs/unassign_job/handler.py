...
def handler(event, context):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 9. Atomic transaction: Unassign provider AND reset applications
            # This ensures data consistency - both updates succeed or both fail
            try:
                # Begin transaction
                conn.begin()
                
                # Step 1: Reset all accepted applications back to pending
                # This allows customers to reconsider all applications
                cur.execute(
                    """
                    UPDATE job_applications 
                    SET status = 'pending'
                    WHERE job_id = %s AND status = 'accepted'
                    """,
                    (job_id,)
                )
                applications_reset = cur.rowcount
                
                # Step 2: Update job status and clear assigned provider
                cur.execute(
                    """
                    UPDATE jobs 
                    SET status = 'open', 
                    ...
                    WHERE job_id = %s AND customer_id = %s AND status = 'assigned'
                    """,
                    (job_id, customer_id)
                )
                ...
                # Rollback on any error - ensures atomic behavior
                conn.rollback()
                print(f"Transaction failed, rolled back: {e}")
                return _response(500, {"message": "Failed to unassign provider. Please try again."})
        ...
        # 11. Format response with applications reset information
        job = {
        ...
        # Construct message based on whether applications were reset
        if applications_reset > 0:
            message = f"Provider unassigned successfully. {applications_reset} application(s) have been reset to pending status."
        else:
        ...
        return _response(200, {
            "message": message,
            "job": job,
            "applications_reset": applications_reset
        })
