...
def handler(event, context):
    """
    Generate presigned S3 URL for job image upload.
    ...
    try:
        ...
        # 9. Verify job exists and belongs to customer
        if not verify_job_ownership(conn, job_id, customer_id):
            return _response(403, {"message": "You do not have permission to upload images to this job"})

        # 10. Check image count limit
        current_count = get_job_images_count(conn, job_id)
        if current_count >= MAX_IMAGES_PER_JOB:
            return _response(400, {
                "message": f"Maximum {MAX_IMAGES_PER_JOB} images per job already reached"
            })
        ...
        s3_key = f"job-images/{job_id}/{timestamp}-{safe_file_name}"
...
if __name__ == "__main__":
    ...
    test_event = {
        "requestContext": {
            "authorizer": {
                "jwt": {
                    "claims": {
                        "sub": "415b3510-a0a1-708e-6a02-dc457aec9ecc"  # Replace with valid customer cognito_sub
                    }
                }
            }
        },
        "pathParameters": {
            "job_id": "1044"  # Replace with valid job_id owned by the customer
        },
        "body": json.dumps({
            "file_name": "kitchen_photo.jpg",
            "content_type": "image/jpeg",
    ...
    print("🔍 Running local test for upload_job_image.handler()...")
