#!/usr/bin/env bash
# Setup API Gateway endpoint for create_review Lambda with JWT authorizer
...
# Check if /reviews resource exists, if not create it
REVIEWS_RESOURCE_ID=$(aws apigateway get-resources \
  --rest-api-id "${API_ID}" \
  --region "${AWS_REGION}" \
  --query "items[?path=='/reviews'].id" \
  --output text)

if [ -z "$REVIEWS_RESOURCE_ID" ]; then
  echo "📝 Creating /reviews resource..."
  REVIEWS_RESOURCE_ID=$(aws apigateway create-resource \
    --rest-api-id "${API_ID}" \
    ...
    --query 'id' \
    --output text)
  echo "✅ Created /reviews resource with ID: ${REVIEWS_RESOURCE_ID}"
else
  echo "✅ Found existing /reviews resource with ID: ${REVIEWS_RESOURCE_ID}"
fi
...
# Create POST method on /reviews
echo "📝 Creating POST method on /reviews..."
...
aws apigateway create-deployment \
  ...
  --description "Deployment for create_review endpoint" \
  --region "${AWS_REGION}"
...
echo "   curl -X POST \\"
echo "     https://${API_ID}.execute-api.${AWS_REGION}.amazonaws.com/prod/reviews \\"
echo "     -H 'Content-Type: application/json' \\"
echo "     -H 'Authorization: Bearer YOUR_JWT_TOKEN' \\"
echo "     -d '{"
echo "       \"job_id\": 1,"
echo "       \"reviewer_id\": 1,"
echo "       \"reviewer_type\": \"customer\","
echo "       \"reviewee_id\": 5,"
echo "       \"reviewee_type\": \"provider\","
echo "       \"rating\": 5,"
echo "       \"comment\": \"Excellent service! Very professional.\""
echo "     }'"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
