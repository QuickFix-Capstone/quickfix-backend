...
STATEMENT_ID="apigateway-create-job-post"
set +e
aws lambda add-permission \
  ...
  --source-arn "arn:aws:execute-api:${AWS_REGION}:${AWS_ACCOUNT_ID}:${API_ID}/*/POST/jobs" \
  --region "${AWS_REGION}" >/tmp/create_job_permission.log 2>&1
PERM_RC=$?
set -e

if [ ${PERM_RC} -ne 0 ]; then
  if rg -q "ResourceConflictException" /tmp/create_job_permission.log; then
    echo "Lambda permission already exists."
  else
    cat /tmp/create_job_permission.log
    exit ${PERM_RC}
  fi
fi

echo "Route configured successfully."
echo "Endpoint: https://${API_ID}.execute-api.${AWS_REGION}.amazonaws.com/prod/jobs"
