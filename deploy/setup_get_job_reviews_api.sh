#!/usr/bin/env bash
# Setup API Gateway endpoint for get_job_reviews Lambda with JWT authorizer
...
# Step 2: Create route
echo "📝 Creating route: GET /reviews/job/{job_id}..."

ROUTE_ID=$(aws apigatewayv2 create-route \
  --api-id "${API_ID}" \
  --route-key "GET /reviews/job/{job_id}" \
...
if [ -z "$ROUTE_ID" ]; then
  ...
  ROUTE_ID=$(aws apigatewayv2 get-routes \
    ...
    --region "${AWS_REGION}" \
    --profile "${AWS_PROFILE}" \
    --query "Items[?RouteKey=='GET /reviews/job/{job_id}'].RouteId" \
    --output text)
...
aws lambda add-permission \
  ...
  --source-arn "arn:aws:execute-api:${AWS_REGION}:${AWS_ACCOUNT_ID}:${API_ID}/*/*/reviews/job/*" \
  --region "${AWS_REGION}" \
  --profile "${AWS_PROFILE}" 2>/dev/null || echo "⚠️  Permission may already exist"
...
echo ""
echo "📍 API Endpoint:"
echo "   GET https://${API_ID}.execute-api.${AWS_REGION}.amazonaws.com/prod/reviews/job/{job_id}"
echo ""
echo "🔐 Authentication:"
...
echo "   curl -X GET \\"
echo "     'https://${API_ID}.execute-api.${AWS_REGION}.amazonaws.com/prod/reviews/job/1001' \\"
echo "     -H 'Authorization: Bearer YOUR_JWT_TOKEN'"
echo ""
echo "📝 Example with query parameters:"
echo "   curl -X GET \\"
echo "     'https://${API_ID}.execute-api.${AWS_REGION}.amazonaws.com/prod/reviews/job/1001?sort=highest_rating&limit=10' \\"
echo "     -H 'Authorization: Bearer YOUR_JWT_TOKEN'"
echo ""
echo "💡 This endpoint uses the same API Gateway and authorizer as other review endpoints"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
