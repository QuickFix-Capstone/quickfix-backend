...
def handler(event, context):
    ...
    read_at = None
    try:
        last_read_message_id, read_at = mark_messages_read(
            messages_table,
            conversation_id=conversation_id,
            reader_user_id=identity["app_user_id"],
        )
    except Exception as exc:
        # Fail soft here; resetting unread count is the primary action.
        print(f"Warning: failed to update read receipts: {exc}")
