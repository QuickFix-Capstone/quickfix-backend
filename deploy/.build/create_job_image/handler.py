...
def handler(event, context):
    """
    Save job image metadata to database after S3 upload.
    ...
    {
      "image_key": "job-images/123/20260201-120000-photo.jpg",
      "image_order": 1,
    ...
    try:
        ...
        # 10. Verify job exists and belongs to customer
        if not verify_job_ownership(conn, job_id, customer_id):
            return _response(403, {"message": "You do not have permission to add images to this job"})

        # 11. Check if image_order is available
        if not is_image_order_available(conn, job_id, image_order):
            return _response(400, {
                "message": f"image_order {image_order} is already used for this job"
            })
        ...
        # 13. Create image record in database
        ...
        if not result['success']:
            return _response(500, {
                "message": f"Failed to save image metadata: {result.get('error', 'Unknown error')}"
            })
...
if __name__ == "__main__":
    ...
    test_event = {
        ...
        "pathParameters": {
            "job_id": "1041"  # Replace with valid job_id owned by the customer
        },
        "body": json.dumps({
            "image_key": "job-images/1041/20260201-172019-bathroom_drain_photo.jpg",
            "image_order": 1,
            "content_type": "image/jpeg",
            "file_size": 1024000,
            "description": "Bathroom drain before repair"
        })
    }

    print("🔍 Running local test for create_job_image.handler()...")
