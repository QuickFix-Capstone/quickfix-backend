import json
from typing import Any, Dict

from src.db.rds_main import get_connection
from pymysql.err import IntegrityError


def _parse_body(event: Dict[str, Any]) -> Dict[str, Any]:
    """
    Support both:
    - API Gateway event: event["body"] is a JSON string
    - Local testing: event itself is already a dict
    """
    if "body" not in event:
        # Assume direct dict for local testing
        return event

    body = event["body"]

    if isinstance(body, dict):
        return body

    if isinstance(body, str):
        try:
            return json.loads(body)
        except json.JSONDecodeError:
            raise ValueError("Invalid JSON body")

    raise ValueError("Unsupported body format")


def _response(status_code: int, body: Dict[str, Any]) -> Dict[str, Any]:
    """Standard API Gateway style response."""
    return {
        "statusCode": status_code,
        "headers": {
            "Content-Type": "application/json",
            "Access-Control-Allow-Origin": "*",
            "Access-Control-Allow-Headers": "Content-Type,Authorization",
            "Access-Control-Allow-Methods": "POST,OPTIONS"
        },
        "body": json.dumps(body),
    }


def _validate_review_data(data: Dict[str, Any]) -> tuple[bool, str]:
    """
    Validate review data according to business rules.

    Returns:
        tuple: (is_valid, error_message)
    """
    # Required fields
    required_fields = ["job_id", "reviewer_id", "reviewer_type",
                      "reviewee_id", "reviewee_type", "rating", "comment"]
    missing = [f for f in required_fields if f not in data]

    if missing:
        return False, f"Missing required fields: {', '.join(missing)}"

    # Validate reviewer_type
    if data["reviewer_type"] not in ["customer", "provider"]:
        return False, "reviewer_type must be 'customer' or 'provider'"

    # Validate reviewee_type
    if data["reviewee_type"] not in ["customer", "provider"]:
        return False, "reviewee_type must be 'customer' or 'provider'"

    # Validate rating range
    try:
        rating = int(data["rating"])
        if rating < 1 or rating > 5:
            return False, "rating must be between 1 and 5"
    except (ValueError, TypeError):
        return False, "rating must be an integer"

    # Validate comment length
    comment = str(data["comment"]).strip()
    if len(comment) < 10:
        return False, "comment must be at least 10 characters"
    if len(comment) > 1000:
        return False, "comment must not exceed 1000 characters"

    # Prevent self-reviews
    if (data["reviewer_id"] == data["reviewee_id"] and
        data["reviewer_type"] == data["reviewee_type"]):
        return False, "You cannot review yourself"

    return True, ""


def handler(event, context):
    """
    Lambda entrypoint for creating a review.

    Expected JSON input (in event["body"] when via API Gateway):

    {
      "job_id": 123,
      "reviewer_id": 1,
      "reviewer_type": "customer",
      "reviewee_id": 5,
      "reviewee_type": "provider",
      "rating": 5,
      "comment": "Excellent service! Very professional and completed the job on time."
    }

    Returns:
        201: Review created successfully
        400: Invalid input data
        409: Duplicate review (reviewer already reviewed this job)
        500: Internal server error
    """
    # 1) Parse and validate input
    try:
        data = _parse_body(event)
    except ValueError as e:
        return _response(400, {"message": str(e)})

    # 2) Validate review data
    is_valid, error_msg = _validate_review_data(data)
    if not is_valid:
        return _response(400, {"message": error_msg})

    # 3) Connect to DB
    conn = get_connection()
    if not conn:
        return _response(500, {"message": "Database connection failed"})

    try:
        with conn.cursor() as cur:
            # Check if job exists and is completed
            cur.execute(
                "SELECT status FROM jobs WHERE job_id = %s",
                (data["job_id"],)
            )
            job = cur.fetchone()

            if not job:
                return _response(404, {"message": "Job not found"})

            if job["status"] != "completed":
                return _response(
                    400,
                    {"message": "Job must be completed before submitting a review"}
                )

            # Insert the review
            sql = """
                INSERT INTO reviews
                (job_id, reviewer_id, reviewer_type, reviewee_id, reviewee_type, rating, comment)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
            """
            cur.execute(
                sql,
                (
                    data["job_id"],
                    data["reviewer_id"],
                    data["reviewer_type"],
                    data["reviewee_id"],
                    data["reviewee_type"],
                    data["rating"],
                    data["comment"].strip(),
                ),
            )
            conn.commit()
            review_id = cur.lastrowid

            # Fetch the created review
            cur.execute(
                "SELECT * FROM reviews WHERE review_id = %s",
                (review_id,)
            )
            review = cur.fetchone()

        # 4) Build success response
        return _response(
            201,
            {
                "message": "Review created successfully",
                "review": {
                    "review_id": review["review_id"],
                    "job_id": review["job_id"],
                    "reviewer_id": review["reviewer_id"],
                    "reviewer_type": review["reviewer_type"],
                    "reviewee_id": review["reviewee_id"],
                    "reviewee_type": review["reviewee_type"],
                    "rating": review["rating"],
                    "comment": review["comment"],
                    "created_at": review["created_at"].isoformat() if review["created_at"] else None,
                },
            },
        )

    except IntegrityError as e:
        error_str = str(e)
        # Duplicate review check
        if "unique_reviewer_job" in error_str.lower() or "duplicate" in error_str.lower():
            return _response(
                409,
                {"message": "You have already submitted a review for this job"},
            )
        # Foreign key constraint violation
        if "foreign key" in error_str.lower():
            return _response(
                400,
                {"message": "Invalid job_id, reviewer_id, or reviewee_id"},
            )
        # Other integrity errors
        return _response(
            400,
            {"message": "Failed to create review due to data constraint"},
        )

    except Exception as e:
        # Generic unexpected error
        print("Unexpected error in create_review:", e)
        return _response(
            500,
            {"message": "Internal server error while creating review"},
        )

    finally:
        try:
            conn.close()
        except Exception:
            pass


# Optional: local test helper
if __name__ == "__main__":
    # Simulate an API Gateway event for quick local testing
    test_event = {
        "body": json.dumps(
            {
                "job_id": 1,
                "reviewer_id": 1,
                "reviewer_type": "customer",
                "reviewee_id": 1,
                "reviewee_type": "provider",
                "rating": 5,
                "comment": "Excellent service! Very professional and completed the job on time. Would definitely hire again.",
            }
        )
    }

    print("🔍 Running local test for create_review.handler()...")
    result = handler(test_event, None)
    print("Response:")
    print(json.dumps(json.loads(result["body"]), indent=2))
    print(f"Status Code: {result['statusCode']}")
