...
SRC_DIR="../lambda/customers/get_customer_public_reviews"
...
if aws lambda get-function --function-name "$FUNC_NAME" --region "$AWS_REGION" > /dev/null 2>&1; then
  ...
  aws lambda create-function \
    ...
    --memory-size 256 \
    --region "$AWS_REGION" \
    --environment "Variables={MYSQL_HOST=quickfix-mysql.cno624c2aarh.us-east-2.rds.amazonaws.com,MYSQL_USER=admin,MYSQL_PASSWORD=QuickFix123!,MYSQL_DB=quickfix,MYSQL_PORT=3306}" > /dev/null
fi
